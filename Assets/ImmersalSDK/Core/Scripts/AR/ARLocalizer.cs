				...
				using (image)
				{
					...
					j.OnResult += (SDKGeoPoseResult result) =>
					{
						...
						if (result.success)
						{
							...
							double[] doubleEnu = new double[4]	// w,x,y,z
                            {
								result.quaternion[1],
								result.quaternion[2],
								result.quaternion[3],
							...
							LocalizerDebugLog(string.Format("GeoPose returned latitude: {0}, longitude: {1}, ellipsoidHeight: {2}, ENU quaternion: {3}, ECEF quaternion: {4}", latitude, longitude, ellipsoidHeight, qEnu, qEcef));
